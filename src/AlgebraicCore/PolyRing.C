...
namespace CoCoA
{
  ...
  {
    if (myIsZero(rawf)) CoCoA_THROW_ERROR(ERR::ZeroRingElem, "RingBase::myIsIrred(rawx)");
  ...
  bool IsSqFree(ConstRefRingElem f)
  {
    ...
    if (!IsPolyRing(P)) CoCoA_THROW_ERROR(ERR::NotPolyRing, "IsSqFree");
    if (!IsField(CoeffRing(P))) CoCoA_THROW_ERROR(ERR::NotField, "IsSqFree");
    if (IsZero(f)) CoCoA_THROW_ERROR(ERR::NotNonZero, "IsSqFree");
    if (IsConstant(f)) return true;
    const bool multivariate = (NumIndets(P) > 1);
    int x = 0;
    if (!multivariate)
      return IsCoprime(f, deriv(f,x));
  ...
  const PolyRing& RingQQt(const MachineInt& NumIndets)
  {
    ...
    if (IsNegative(NumIndets) || IsZero(NumIndets)) CoCoA_THROW_ERROR(ERR::NotPositive, "RingQQt");
  ...
  {
    if (!IsPolyRing(owner(f)))
      CoCoA_THROW_ERROR(ERR::NotElemPolyRing, "IsMonomial(f)");
...
//       == v.end(); 
  ...
  {
    if (!IsPolyRing(owner(f)))
      CoCoA_THROW_ERROR(ERR::NotElemPolyRing, "IsConstant(f)");
  ...
  {
    if (!IsPolyRing(owner(f)))
      CoCoA_THROW_ERROR(ERR::NotElemPolyRing, "IsIndet(f)");
    long junk;
  ...
  {
    if (!IsPolyRing(owner(f)))
      CoCoA_THROW_ERROR(ERR::NotElemPolyRing, "IsIndet(index,f)");
  ...
  {
    if (!IsPolyRing(owner(f)))
      CoCoA_THROW_ERROR(ERR::NotElemPolyRing, "IsIndetPosPower(f)");
  ...
  {
    if (!IsPolyRing(owner(f)))
      CoCoA_THROW_ERROR(ERR::NotElemPolyRing, "IsEvenPoly(f)");
  ...
  {
    if (!IsPolyRing(owner(f)))
      CoCoA_THROW_ERROR(ERR::NotElemPolyRing, "IsOddPoly(f)");
  ...
  long deg(ConstRefRingElem f, long var)
  {
    ...
    if (!IsPolyRing(owner(f)))
      CoCoA_THROW_ERROR(ERR::NotElemPolyRing, FnName);
  ...
  RingElem FixedDivisor(ConstRefRingElem f)
  {
    ...
    if (IsZero(f)) CoCoA_THROW_ERROR(ERR::BadArg, "FixedDivisor: arg must be non-zero");
    ...
    long x_lo = 0;
    RingElem v = EvalHom(P,EvalPt)(f);
    BigInt val;
    if (!IsInteger(val, v)) CoCoA_THROW_ERROR(ERR::BadArg, "FixedDivisor: expected integer value");
    BigInt val_lo = abs(val)/cont;
    BigInt IC = gcd(val_lo, factorial(deg(f)));  // wasteful if val_lo is small!
    if (IsOne(IC)) return RingElem(k,cont);

    long x_hi = 0;
    BigInt val_hi = val_lo;
    long width=1;
    ...
    while (true)
    {
      ...
      v = EvalHom(P,EvalPt)(f);
      if (!IsInteger(val, v)) CoCoA_THROW_ERROR(ERR::BadArg, "FixedDivisor: expected integer value");
      IC = gcd(IC, val/cont);
      if (IsOne(IC)) return RingElem(k,cont);
      if (IC <= fact  &&  gcd(IC,fact) == IC) return RingElem(k,IC*cont);
      if (MoveHi)
      {
        val_hi = abs(val);
      }
  ...
  namespace // anonymous for file local fn
  {
    RingElem DerivFrF(ConstRefRingElem f, ConstRefRingElem x)
    {
      ...
      if (!IsPolyRing(BaseRing(FrF))) CoCoA_THROW_ERROR(ERR::NotPolyRing, "deriv(f,x)");
  ...
  RingElem deriv(ConstRefRingElem f, ConstRefRingElem x)
  {
    ...
    if (!IsIndet(x)) CoCoA_THROW_ERROR(ERR::NotIndet, "deriv(f,x)");
...
// $Header: /Volumes/Home_1/cocoa/cvs-repository/CoCoALib-0.99/src/AlgebraicCore/PolyRing.C,v 1.73 2022/02/18 14:11:56 abbott Exp $
// $Log: PolyRing.C,v $
