...
//???    myDenFactors(den.myFactors, den.myMultiplicities, den.myRemainingFactor)
  {
    ...
    if (!den.myFactors().empty() && (owner(num) != owner(den.myRemainingFactor())))
      CoCoA_THROW_ERROR(ERR::MixedRings, "HPSeries::HPSeries");
  }
...
///???    myDenFactors(vector<RingElem>(), vector<long>(), one(RingQQt(1)))
  ...
  RingElem HilbertNumQuot_C(const ideal& I)
  {
    ...
    for (const auto& g: GB)
      if ( !IsHomog(g) )
        CoCoA_THROW_ERROR("GBasis not homogeneous", "HilbertNumQuot_C");
  ...
  RingElem MGHilbertNumQuot(const ideal& I)
  {
    ...
    if (!IsHomog(I))  CoCoA_THROW_ERROR("not homogeneous", "MGHilbertNumQuot");
  ...
  HPSeries HilbertSeriesQuot(const ideal& I)
  {
    ...
    if (!HasPositiveGrading(RingOf(I)))
      CoCoA_THROW_ERROR(ERR::NotPositiveGrading, "HilbertSeriesQuot");
...
//       CoCoA_THROW_ERROR("HPSeries must be standard");
  ...
  long DimQuot(const ideal& I)
  {
    if (!IsStdGraded(RingOf(I)))
      CoCoA_THROW_ERROR("Ring is not standard graded", "dim");
    if (AreGensMonomial(I)) return dim(HilbertSeriesQuot(radical_tmp(I)));
  ...
  {
    if (!IsStdGraded(RingOf(I)))
      CoCoA_THROW_ERROR("Ring is not standard graded", "multiplicity");
  ...
  {
//     if (!IsStdGraded(P))
//       CoCoA_THROW_ERROR("Ring is not standard graded", "HilbertPolyQuot");
...
// $Header: /Volumes/Home_1/cocoa/cvs-repository/CoCoALib-0.99/src/AlgebraicCore/SparsePolyOps-hilbert.C,v 1.12 2022/02/18 14:11:59 abbott Exp $
