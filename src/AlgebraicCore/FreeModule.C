...
namespace CoCoA
{
  ...
  const FreeModuleBase* FreeModulePtr(const module& M, const char* const FnName)
  {
    ...
    if (ptr == nullptr) CoCoA_THROW_ERROR(ERR::NotFreeModule, FnName);
    return ptr;
  }
...
///???    bool IsZeroAddMul(RawPtr& rawlhs, RingElemConstRawPtr rawy, ConstRawPtr rawz) const;  // lhs += y*z, result says whether lhs == 0.
  ...
  {
    if (NumCompts < 0)
      CoCoA_THROW_ERROR(ERR::NotNonNegative, "NewFreeModule(R,n)");
  ...
  long FirstNonZeroPosn(const ModuleElem& v)
  {
    ...
    CoCoA_THROW_ERROR("ModuleElem is zero", "FirstNonZeroPosn(ModuleElem)");
  ...
  RingElem FirstNonZero(const ModuleElem& v)
  {
    ...
    CoCoA_THROW_ERROR("ModuleElem is zero", "FirstNonZero(ModuleElem)");
...
//???    ~FreeModuleSpPRImpl();  // can be handy for debugging
      ...
      myOrdPtr()
  {
    if ( IsWDegPosnOrd(O) )  myOrdPtr.reset(new WDegPosnOrdImpl(shifts(O)));
      ...
      CoCoA_THROW_ERROR("ModuleOrdering not implemented", "GradedModuleCmpBase");
  }
  ...
  {
    if (NumCompts < 0)
      CoCoA_THROW_ERROR(ERR::NotNonNegative, "NewFreeModuleSpPR(P,n)");
  ...
  FreeModule NewFreeModule(const ring& P, const std::vector<degree>& shifts)
  {
    if (!IsSparsePolyRing(P))
      CoCoA_THROW_ERROR(ERR::NotSparsePolyRing, "NewFreeModule(R, Ord)");
  ...
  {
    if (!IsSparsePolyRing(P))
      CoCoA_THROW_ERROR(ERR::NotSparsePolyRing, "NewFreeModule(R, shifts, Ord)");
  ...
  {
    if (!IsSparsePolyRing(P))
      CoCoA_THROW_ERROR(ERR::NotSparsePolyRing, "NewFreeModule(R, NumCompts, Ord)");
  ...
  const ModuleOrdering& ordering(const FreeModule& M)
  {
    if (!IsFreeModuleSpPR(M))
      CoCoA_THROW_ERROR(ERR::NotModuleSpPR, "ordering(M)");
  ...
  {
    if (!IsFreeModuleSpPR(M))
      CoCoA_THROW_ERROR(ERR::NotModuleSpPR, "shifts(M)");
  ...
  long LPosn(const ModuleElem& v)
  {
    if ( !IsFreeModuleSpPR(owner(v)) )
      CoCoA_THROW_ERROR(ERR::NotModuleSpPR, "LPosn(v)");
  ...
  {
    if ( !IsFreeModuleSpPR(owner(v)) )
      CoCoA_THROW_ERROR(ERR::NotModuleSpPR, "LPP(v)");
  ...
  {
    if ( !IsFreeModuleSpPR(owner(v)) )
      CoCoA_THROW_ERROR(ERR::NotModuleSpPR, "LC(v)");
  ...
  {
    if ( !IsFreeModuleSpPR(owner(v)) )
      CoCoA_THROW_ERROR(ERR::NotModuleSpPR, "wdeg(v)");
  ...
  {
    if ( !IsFreeModuleSpPR(owner(v)) )
      CoCoA_THROW_ERROR(ERR::NotModuleSpPR, "deg(v)");
    if (IsZero(v)) CoCoA_THROW_ERROR(ERR::NotNonZero, "StdDeg(v)");
    long PolyDegree = 0;
  ...
  int CmpWDeg(const ModuleElem& v1, const ModuleElem& v2)
  {
    if ( owner(v1) != owner(v2) )
      CoCoA_THROW_ERROR(ERR::MixedModules, "CmpWDeg(v1, v2)");
    if ( !IsFreeModuleSpPR(owner(v1)) )
      CoCoA_THROW_ERROR(ERR::NotModuleSpPR, "CmpWDeg(v1, v2)");
  ...
  {
    if (!IsFreeModuleSpPR(owner(v)))
      CoCoA_THROW_ERROR(ERR::NotModuleSpPR, "ModuleElem(v, h)");
    if ( GradingDim(RingOf(owner(v)))!=1 )
      CoCoA_THROW_ERROR(ERR::NYI, "ModuleElem(v, h) GradingDim!=1");
  ...
  {
    if ( !IsFreeModuleSpPR(owner(v)) )
      CoCoA_THROW_ERROR(ERR::NotModuleSpPR, "IsHomog(v)");
  ...
  {
    if (L.empty()) CoCoA_THROW_ERROR(ERR::Empty, "NewFreeModuleForSyz");
    if (!IsSparsePolyRing(owner(L[0])))
      CoCoA_THROW_ERROR(ERR::NYI, "NewFreeModuleForSyz non polynomials");
...
// ORIG    transform(L.begin(), L.end(), back_inserter(sh),
// ORIG             ptr_fun(static_cast<degree (*)(ConstRefRingElem)>(wdeg)));    
...
// $Header: /Volumes/Home_1/cocoa/cvs-repository/CoCoALib-0.99/src/AlgebraicCore/FreeModule.C,v 1.42 2022/02/18 15:38:24 abbott Exp $
// $Log: FreeModule.C,v $
// Revision 1.42  2022/02/18 15:38:24  abbott
// Summary: Using lambda fn to avoid clang "deprecated" warning
//
// Revision 1.41  2022/02/18 14:11:54  abbott
// Summary: Updated copyright notice (now restrictive; see redmine 1555)
//
// Revision 1.40  2022/02/08 20:18:54  abbott
// Summary: Renamed OpenMath output fns (added suffix _OM) (redmine 1528)
//
// Revision 1.39  2022/02/05 19:57:32  abbott
// Summary: Added CanonicalBasis (redmine 1235)
//
// Revision 1.38  2021/10/30 16:48:27  abbott
// Summary: Used keywords override and delete (redmine 1625 & 1627)
//
// Revision 1.37  2021/01/07 15:07:03  abbott
// Summary: Corrected copyright
//
// Revision 1.36  2020/06/17 15:49:23  abbott
// Summary: Changed CoCoA_ERROR into CoCoA_THROW_ERROR
//
// Revision 1.35  2020/02/11 16:56:40  abbott
// Summary: Corrected last update (see redmine 969)
//
// Revision 1.34  2020/02/11 16:12:18  abbott
// Summary: Added some checks for bad ostream (even to mem fns myOutput); see redmine 969
//
// Revision 1.33  2019/03/19 11:07:07  abbott
// Summary: Replaced 0 by nullptr where appropriate
//
// Revision 1.32  2019/03/04 10:29:04  abbott
// Summary: Changed auto_ptr into unqiue_ptr
//
// Revision 1.31  2018/08/01 15:06:55  bigatti
// -- removed commented code in NewFreeModuleForSyz (module)
//
// Revision 1.30  2018/05/25 09:24:46  abbott
// Summary: Major redesign of CpuTimeLimit (many consequences)
//
// Revision 1.29  2018/05/18 16:40:27  bigatti
// -- added include SparsePolyOps-RingElem.H
//
// Revision 1.28  2018/05/18 12:13:36  bigatti
// -- renamed IntOperations --> BigIntOps
//
// Revision 1.27  2015/12/01 13:11:00  abbott
