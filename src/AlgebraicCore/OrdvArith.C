...
//    if (ordv1 == ordv2) return; // worth checking this special case???
  ...
  void OrdvArith::base::myPower(OrdvElem* ordv, const OrdvElem* ordv1, long LongExp) const
  {
    ...
    if (exp > myOrdvMask)
      CoCoA_THROW_ERROR(ERR::ExpTooBig, "OrdvArith::myPower");
    for (long i=0; i < myOrdvWords; ++i)
      ordv[i] = exp*ordv1[i];
  }
  ...
  void OrdvArith::XelImpl::myComputeExpv(vector<long>& expv, const OrdvElem* ordv) const
  {
    ...
    myDecompress(myOrdvBuffer, ordv, myNumIndets);
    for (long i=0; i < myNumIndets; ++i)
      expv[i] = NumericCast<long>(myOrdvBuffer[myNumIndets-i-1]);
  }


  long OrdvArith::XelImpl::myExponent(const OrdvElem* ordv, long var) const
  {
    return NumericCast<long>(myOrdvGetNth(ordv, myNumIndets-var-1));
  }
  ...
  {
    CoCoA_THROW_ERROR(ERR::NYI, "MatrixOrderingImpl");
...
//     out << "]), ";
    ...
    base(NumIndets, GradingDim, NumIndets)
  {
    ...
    if (!IsTermOrdering(OrderMatrix))
      CoCoA_THROW_ERROR(ERR::NotTermOrdering, "OrdvArith::MatrixOrderingMod32749Impl::MatrixOrderingMod32749Impl");

    const QuotientRing Fp = NewRingFp(32749, GlobalSettings::ResidueRepr::NonNegative);
    ...
    for (long i=0; i < GradingDim; ++i)
      for (long j=0; j < NumCols(OrderMatrix); ++j)
        if (sign(OrderMatrix(i,j)) < 0)
          CoCoA_THROW_ERROR(ERR::NYI, "MatrixOrderingMod32749Impl: temporarily requiring weights to be non-negative");
    matrix PosOrdMat = MakeTermOrdMat(OrderMatrix);
    for (long i=0; i < myNumIndets; ++i)
      for (long j=0; j < myNumIndets; ++j)
      {
        if (PosOrdMat(i,j)>= 32749) CoCoA_THROW_ERROR(ERR::ArgTooBig, "OrdvArith::MatrixOrderingMod32749Impl ctor");
...
// RCS header/log
// $Header: /Volumes/Home_1/cocoa/cvs-repository/CoCoALib-0.99/src/AlgebraicCore/OrdvArith.C,v 1.58 2022/02/23 12:38:25 abbott Exp $
// $Log: OrdvArith.C,v $
// Revision 1.58  2022/02/23 12:38:25  abbott
// Summary: Improved err mesg (req coords in gr dim rows to be non neg)
//
// Revision 1.57  2022/02/18 14:11:56  abbott
// Summary: Updated copyright notice (now restrictive; see redmine 1555)
//
// Revision 1.56  2022/02/04 21:41:06  abbott
// Summary: Changed name MakeTermOrd to MakeTermOrdMat (redine 854)
//
// Revision 1.55  2021/03/04 21:03:45  abbott
// Summary: enum revision and renaming (redmine 894)
//
// Revision 1.54  2021/03/03 22:09:32  abbott
// Summary: New enum class (redmine 894)
//
// Revision 1.53  2021/02/22 21:09:25  abbott
// Summary: Implemented xel (redmine 1536)
//
// Revision 1.52  2021/01/07 15:16:51  abbott
// Summary: Corrected copyright
//
// Revision 1.51  2020/06/17 15:49:24  abbott
// Summary: Changed CoCoA_ERROR into CoCoA_THROW_ERROR
//
// Revision 1.50  2020/02/11 16:56:41  abbott
// Summary: Corrected last update (see redmine 969)
//
// Revision 1.49  2020/02/11 16:12:18  abbott
