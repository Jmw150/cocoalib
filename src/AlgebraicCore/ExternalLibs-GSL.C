...
namespace CoCoA
{
    ...
    for (int i=0; i < rows; ++i)
    {
      for (int j=0; j < cols; ++j)
      {
        ...
        if (!IsRational(k, elem))
          CoCoA_THROW_ERROR("Cannot convert to double", "NewMatrixGSL");
  ...
  gsl_vector* NewVectorGSL(std::vector<RingElem>& v)
  {
    ...
    for (long j=0; j < cols; ++j)
    {
      RingElem elem = v[j];
      if (!IsRational(k, elem))
        CoCoA_THROW_ERROR("Cannot convert to double", "NewVectorGSL");
  ...
  std::vector<RingElem> GslSingularValues(ConstMatrixView A)
  {
    ...
    if(status != GSL_SUCCESS)
      CoCoA_THROW_ERROR(gsl_strerror (status), "SingularValues");
  ...
  std::vector<matrix> GslSVD(ConstMatrixView A)
  {
    ...
    int status = gsl_linalg_SV_decomp(m, q, d, w);   
    if(status != GSL_SUCCESS)
      CoCoA_THROW_ERROR(gsl_strerror (status), "SVD");
  ...
  std::vector<RingElem> GslSolveSVD(ConstMatrixView A, std::vector<RingElem>& b)
  {
    ...
    int status = gsl_linalg_SV_decomp(m, q, d, w);   
    if(status != GSL_SUCCESS)
      CoCoA_THROW_ERROR(gsl_strerror (status), "SolveSVD");
  ...
  matrix GslLU(ConstMatrixView A)
  {
    ...
    int cols = NumCols(A);
    if (NumRows(A) != cols)
      CoCoA_THROW_ERROR(ERR::NotSquareMatrix, "LU");

    if (0 == cols)
      CoCoA_THROW_ERROR(ERR::BadMatrixSize, "LU");
    ...
    if(status != GSL_SUCCESS)
      CoCoA_THROW_ERROR(gsl_strerror (status), "LU");
  ...
  std::vector<RingElem> GslSolverLU(ConstMatrixView A, std::vector<RingElem>& b)
  {
    ...
    const long cols = NumCols(A);
    if (NumRows(A) != cols)
      CoCoA_THROW_ERROR(ERR::NotSquareMatrix, "SolverLU");

    if (0 == cols || cols != len(b))
      CoCoA_THROW_ERROR(ERR::BadMatrixSize, "SolverLU");
    ...
    int status = gsl_linalg_LU_decomp (m, p, &s);
    if(status != GSL_SUCCESS)
      CoCoA_THROW_ERROR(gsl_strerror (status), "SolverLU");
  ...
  matrix GslCholeskyDecomposition(ConstMatrixView A)
  {
    ...
    if(status != GSL_SUCCESS)
      CoCoA_THROW_ERROR(gsl_strerror (status), "CholeskyDecomposition");
  ...
  std::vector<matrix> GslQR(ConstMatrixView A)
  {
    ...
    if(status != GSL_SUCCESS)
      CoCoA_THROW_ERROR(gsl_strerror (status), "QR");
  ...
  std::vector<matrix> GslQRPT(ConstMatrixView A)
  {
    ...
    if(status != GSL_SUCCESS)
      CoCoA_THROW_ERROR(gsl_strerror (status), "QRPT");
  ...
  std::vector<RingElem> GslSolverQR(ConstMatrixView A, std::vector<RingElem>& b)
  {
    ...
    int status = gsl_linalg_QR_decomp(m, gsl_tau);
    if(status != GSL_SUCCESS)
      CoCoA_THROW_ERROR(gsl_strerror (status), "SolverQR");
  ...
  void QRDecomposition(ConstMatrixView A, matrix& QR, std::vector<RingElem>& tau)
  {
    ...
    int status = gsl_linalg_QR_decomp(m, gsl_tau);
    if(status != GSL_SUCCESS)
      CoCoA_THROW_ERROR(gsl_strerror (status), "QRDecomposition");
  ...
  void GslBidiagDecomposition(ConstMatrixView A, matrix& D, std::vector<RingElem>& tau_U, std::vector<RingElem>& tau_V)
  {
    ...
    if(status != GSL_SUCCESS)
      CoCoA_THROW_ERROR(gsl_strerror (status), "BidiagDecomposition");
  ...
  void GslBidiagDecompUnpack(ConstMatrixView A, matrix& U, matrix& V, std::vector<RingElem>& diagonal, std::vector<RingElem>& superdiagonal)
  {
    ...
    int status = gsl_linalg_bidiag_decomp (m, gsl_tau_U, gsl_tau_V);
    if(status != GSL_SUCCESS)
      CoCoA_THROW_ERROR(gsl_strerror (status), "BidiagDecompUnpack");
...
// $Header: /Volumes/Home_1/cocoa/cvs-repository/CoCoALib-0.99/src/AlgebraicCore/ExternalLibs-GSL.C,v 1.20 2022/02/18 14:11:54 abbott Exp $
// $Log: ExternalLibs-GSL.C,v $
// Revision 1.20  2022/02/18 14:11:54  abbott
// Summary: Updated copyright notice (now restrictive; see redmine 1555)
//
// Revision 1.19  2021/10/04 08:52:51  abbott
// Summary: Replaced calls to apply by direct applic of ringhom (redmine 1467, 1598)
//
// Revision 1.18  2021/07/19 11:08:25  abbott
// Summary: Layout changes; and prefix incr instead of postfix
//
// Revision 1.17  2020/06/17 15:49:22  abbott
// Summary: Changed CoCoA_ERROR into CoCoA_THROW_ERROR
//
// Revision 1.16  2020/01/26 14:33:08  abbott
// Summary: Removed useless include
//
// Revision 1.15  2019/03/18 11:14:33  abbott
// Summary: Added include for iostream after changing NumTheory
//
// Revision 1.14  2017/03/13 12:30:37  abbott
